...
#[tokio::test]
async fn test_presence_channel_join_uses_resolved_channel_when_message_channel_is_missing() {
    let adapter = Arc::new(LocalAdapter::new()) as Arc<dyn Adapter>;
    adapter.init().await.unwrap();
    let manager = PresenceChannelManager::new(adapter.clone());
    ...
    let channel = "presence-tenant.01ks2dffp902d2mymhey89ze1c.chat.01KS2DWBDCX9DN2KMJXBXZ9GJA";
    let channel_data = json!({
        "user_id": "user-123",
        "user_info": {
            "name": "Test User"
