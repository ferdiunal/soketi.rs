...
use soketi_rs::channels::encrypted::EncryptedPrivateChannelManager;
...
/// Helper function to generate a valid auth signature for encrypted private channels
...
#[tokio::test]
async fn test_encrypted_channel_join_with_valid_auth() {
    // Create adapter and channel manager
    let adapter = Arc::new(LocalAdapter::new()) as Arc<dyn Adapter>;
    adapter.init().await.unwrap();
    let manager = EncryptedPrivateChannelManager::new(adapter.clone());
    ...
    let channel = "private-encrypted-test-channel";
    ...
    // Join the encrypted private channel
...
#[tokio::test]
async fn test_encrypted_channel_join_uses_resolved_channel_when_message_channel_is_missing() {
    let adapter = Arc::new(LocalAdapter::new()) as Arc<dyn Adapter>;
    adapter.init().await.unwrap();
    let manager = EncryptedPrivateChannelManager::new(adapter.clone());
    ...
    let channel =
        "private-encrypted-tenant.01ks2dffp902d2mymhey89ze1c.chat.01KS2DWBDCX9DN2KMJXBXZ9GJA";
    let auth = generate_auth_signature(&app.key, &app.secret, &socket.id, channel);
...
#[tokio::test]
async fn test_encrypted_channel_join_with_invalid_auth() {
    // Create adapter and channel manager
    let adapter = Arc::new(LocalAdapter::new()) as Arc<dyn Adapter>;
    adapter.init().await.unwrap();
    let manager = EncryptedPrivateChannelManager::new(adapter.clone());
    ...
    // Try to join the encrypted private channel
    let response = manager.join(&app, &socket, channel, Some(message)).await;
...
#[tokio::test]
async fn test_encrypted_channel_join_without_auth() {
    // Create adapter and channel manager
    let adapter = Arc::new(LocalAdapter::new()) as Arc<dyn Adapter>;
    adapter.init().await.unwrap();
    let manager = EncryptedPrivateChannelManager::new(adapter.clone());
...
#[tokio::test]
async fn test_encrypted_channel_join_without_message() {
    // Create adapter and channel manager
    let adapter = Arc::new(LocalAdapter::new()) as Arc<dyn Adapter>;
    adapter.init().await.unwrap();
    let manager = EncryptedPrivateChannelManager::new(adapter.clone());
...
#[tokio::test]
async fn test_encrypted_channel_join_with_wrong_secret() {
    // Create adapter and channel manager
    let adapter = Arc::new(LocalAdapter::new()) as Arc<dyn Adapter>;
    adapter.init().await.unwrap();
    let manager = EncryptedPrivateChannelManager::new(adapter.clone());
...
#[tokio::test]
async fn test_encrypted_channel_join_with_wrong_socket_id() {
    // Create adapter and channel manager
    let adapter = Arc::new(LocalAdapter::new()) as Arc<dyn Adapter>;
    adapter.init().await.unwrap();
    let manager = EncryptedPrivateChannelManager::new(adapter.clone());
...
#[tokio::test]
async fn test_encrypted_channel_join_with_wrong_channel() {
    // Create adapter and channel manager
    let adapter = Arc::new(LocalAdapter::new()) as Arc<dyn Adapter>;
    adapter.init().await.unwrap();
    let manager = EncryptedPrivateChannelManager::new(adapter.clone());
    ...
    let auth = generate_auth_signature(
        ...
        &app.secret,
        &socket.id,
        "private-encrypted-different-channel",
    );
...
#[tokio::test]
async fn test_encrypted_channel_multiple_sockets_with_valid_auth() {
    // Create adapter and channel manager
    let adapter = Arc::new(LocalAdapter::new()) as Arc<dyn Adapter>;
    adapter.init().await.unwrap();
    let manager = EncryptedPrivateChannelManager::new(adapter.clone());
...
#[tokio::test]
async fn test_encrypted_channel_join_different_channels() {
    // Create adapter and channel manager
    let adapter = Arc::new(LocalAdapter::new()) as Arc<dyn Adapter>;
    adapter.init().await.unwrap();
    let manager = EncryptedPrivateChannelManager::new(adapter.clone());
    ...
    // Join multiple different encrypted private channels
    let channels = vec![
        "private-encrypted-channel-1",
        "private-encrypted-channel-2",
        "private-encrypted-channel-3",
    ];
...
#[tokio::test]
async fn test_encrypted_channel_auth_signature_format() {
    // Create adapter and channel manager
    let adapter = Arc::new(LocalAdapter::new()) as Arc<dyn Adapter>;
    adapter.init().await.unwrap();
    let manager = EncryptedPrivateChannelManager::new(adapter.clone());
...
#[tokio::test]
async fn test_encrypted_channel_behaves_like_private_channel() {
    // This test verifies that encrypted channels use the same authentication
    // mechanism as private channels. The encryption is client-side only.
    ...
    // Generate valid auth signature (same as private channels)
    let channel = "private-encrypted-test-channel";
    let auth = generate_auth_signature(&app.key, &app.secret, &socket.id, channel);
    ...
    // Verify success - encrypted channels work exactly like private channels
    assert!(
        response.success,
        "Encrypted channels should authenticate like private channels"
    );
    assert_eq!(response.error_code, None);
    assert_eq!(response.auth_error, false);
    ...
    let is_in = adapter
        ...
        .await
        .unwrap();
    assert!(is_in, "Socket should be in the encrypted channel");
}
